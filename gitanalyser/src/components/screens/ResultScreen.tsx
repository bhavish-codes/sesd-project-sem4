        ...
        <div className="flex flex-col items-end gap-4">
          <div className="w-24 h-24 absolute top-4 right-4 pointer-events-none opacity-80">
            <svg viewBox="0 0 100 100" className="w-full h-full text-black">
              <path id="circlePath3" d="M 50, 50 m -35, 0 a 35,35 0 1,1 70,0 a 35,35 0 1,1 -70,0" fill="transparent" />
          ...
          <div className="mt-16 mr-12 text-xs font-bold font-mono tracking-widest text-gray-700">TARGET ALIAS <span className="float-right">[REQ. 01]</span></div>
          <div className="flex border-[3px] border-black bg-white mr-12 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
            <div className="px-4 py-2 font-mono text-lg font-bold w-48 text-gray-700">torvalds</div>
            <div className="bg-black text-white px-8 py-2 font-black font-mono flex items-center justify-center cursor-pointer hover:bg-gray-800 uppercase text-lg" onClick={() => setScreen("MULTI_RESULT")}>
              Scan
            </div>
          </div>
        </div>
        ...
        <div className="w-1/3 min-w-[280px] border-r-[4px] border-black flex flex-col p-6">
          <div className="border-[3px] border-black p-2 bg-white relative mb-6 shadow-[6px_6px_0_rgba(0,0,0,0.1)]">
            ...
            <div className="aspect-square bg-gray-300 relative overflow-hidden grayscale">
              <picture>
                <img src={`https://github.com/${data?.username || 'torvalds'}.png`} alt="Profile" className="w-full h-full object-cover" />
              </picture>
            </div>
          </div>
          ...
          <div className="border-t-[3px] border-black py-4">
            <p className="font-mono text-xs leading-relaxed text-gray-800">
              Creator of Linux and Git.<br />
              "Talk is cheap. Show me the code."
            </p>
          </div>
          
          <div className="font-mono text-xs font-bold tracking-widest mb-6">
            [LOC: HELSINKI/PORTLAND]
          </div>
          
          <div className="border-b-[3px] border-black mb-6"></div>

          <div className="flex flex-wrap gap-2 mb-4">
            <span className="border-[2px] border-black px-2 py-1 text-[10px] font-bold tracking-widest uppercase">Linux Foundation</span>
            <span className="border-[2px] border-black px-2 py-1 text-[10px] font-bold tracking-widest uppercase">Kernel.org</span>
          </div>
        </div>
        ...
        <div className="w-2/3 flex flex-col p-6 space-y-6">
          ...
          <div className="flex gap-4 border-[3px] border-black divide-x-[3px] divide-black shrink-0 relative overflow-hidden">
            ...
            {[
              ...
              <div key={i} className="flex-1 py-6 flex flex-col items-center justify-center bg-[#eae6dc] z-10">
                <div className="font-['Playfair_Display',var(--font-serif)] text-5xl font-bold text-[var(--color-brand-red)] mb-4">{stat.val}</div>
                <div className="font-mono text-xs tracking-widest text-black font-black">{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="border-[3px] border-black p-4 bg-[#eae6dc]">
            <div className="flex justify-between items-end border-b-[2px] border-black pb-2 mb-4">
              <div className="font-['Playfair_Display',var(--font-serif)] font-bold text-xl tracking-wide uppercase">Linguistic Formula</div>
              <div className="text-[var(--color-brand-red)] font-mono text-[10px] tracking-widest font-black">[VOLUMETRIC BREAKDOWN]</div>
            </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={item.lang} className="flex items-center gap-4">
                  <div className="w-24 font-mono text-xs font-bold tracking-widest truncate">{item.lang}</div>
                  <div className="flex-1 h-5 border-[1px] border-black flex gap-px bg-[#d1cec6] p-px">
                     <div className="h-full bg-[var(--color-brand-red)]" style={{ width: `${item.pct}%` }}></div>
                     <div className="flex-1 flex opacity-20">
                       {/* Faux grid sections inside the empty bar */}
                       {Array.from({length: 10}).map((_, i) => (
                         <div key={i} className="flex-1 border-r border-[#666]"></div>
                       ))}
                     </div>
                  </div>
                  <div className="w-10 text-right font-mono text-xs font-bold">{item.pct}%</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="border-[3px] border-black p-4 bg-[#eae6dc] flex-1 flex flex-col">
            <div className="flex justify-between items-end border-b-[2px] border-black pb-2 mb-4">
              <div className="font-['Playfair_Display',var(--font-serif)] font-bold text-xl tracking-wide uppercase">Extraction Matrix</div>
              <div className="text-[var(--color-brand-red)] font-mono text-[10px] tracking-widest font-black">[LAST 90 DAYS]</div>
            </div>
