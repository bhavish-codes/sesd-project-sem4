...
export function LoadingScreen({ setScreen, setData }: Props) {
        ...
        <div className="flex flex-col items-end gap-4">
          <div className="w-24 h-24 absolute top-4 right-4 pointer-events-none opacity-80 animate-spin-slow">
            <svg viewBox="0 0 100 100" className="w-full h-full">
              ...
              <text className="text-[12px] font-mono font-bold tracking-widest" fill="black">
          ...
          <div className="mt-16 mr-12 text-xs font-bold font-mono tracking-widest text-gray-700">TARGET ALIAS <span className="float-right">[SCANNING...]</span></div>
          <div className="flex border-[3px] border-black bg-white mr-12">
            <div className="px-4 py-2 font-mono text-lg font-bold w-48 text-gray-500">torvalds</div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/3 border-r-[4px] border-black flex flex-col p-6 relative">
          ...
          <div className="w-full aspect-square border-[3px] border-black relative bg-[#e2ddd1] flex items-center justify-center overflow-hidden mb-8">
            ...
            <div className="w-12 h-12 border-2 border-[var(--color-brand-red)] rounded-full relative z-10 flex items-center justify-center">
        ...
        <div className="w-2/3 flex flex-col p-6">
          ...
          <div className="flex gap-4 border-[3px] border-black mb-6 divide-x-[3px] divide-black">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex-1 py-8 flex flex-col items-center justify-center bg-[#eae6dc]">
                <div className="w-12 h-4 bg-black mb-4"></div>
                <div className="font-mono text-sm tracking-widest text-[var(--color-brand-red)] font-black">PENDING</div>
              </div>
            ))}
          </div>

          {/* Decoding schema area */}
          <div className="flex-1 border-[3px] border-black relative p-4 flex flex-col">
            ...
            <div className="flex flex-wrap gap-2 opacity-10 pointer-events-none mb-auto">
              {Array.from({length: 36}).map((_, i) => (
                <div key={i} className="w-8 h-8 bg-[var(--color-brand-red)]"></div>
              ))}
            </div>
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black border-4 border-black border-r-[var(--color-brand-red)] border-b-[var(--color-brand-red)] text-white px-8 py-4 font-mono z-10 whitespace-nowrap shadow-[8px_8px_0_rgba(0,0,0,0.2)]">
              <span className="text-xl font-bold tracking-wider">TARGET ACQUISITION IN PROGRESS</span>
            </div>
            
            {/* Red Scanning line */}
            <div className="absolute top-48 -left-full right-[-100vw] h-0.5 bg-[var(--color-brand-red)] z-20 shadow-[0_0_10px_rgba(187,42,40,0.5)]"></div>
